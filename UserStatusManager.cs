...
namespace TootTallyAccounts
{
    public class UserStatusManager : MonoBehaviour
    {
        ...
        {
            if (!_isInitialized || TootTallyUser.userInfo.id == 0) return;
        ...
        {
            _isTimerReady = false;
            Plugin.Instance.StartCoroutine(TootTallyAPIService.SendUserStatus(Plugin.GetAPIKey, (int)_currentStatus, OnHeartBeatRequestResponse));
        }
